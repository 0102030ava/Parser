...
def generate_json(link,page_id,id):
   ...
   if "search_text" not in gen_link:
         headers = {
            ...
            'dnt': '1',
            'referer': 'https://www.vinted.pl/vetements?catalog[]='+str(catalog_id)+'&page='+str(page_id+1),
      ...
      headers = {
            ...
            'accept-language': 'it-fr',           
            'dnt': '1',
            'referer': 'https://www.vinted.pl/vetements?search_text=head&currency=EUR&search_id=6350979173&order=newest_first&time=1662612074&page=2',
            'sec-ch-ua': '"Chromium";v="104", " Not A;Brand";v="99", "Google Chrome";v="104"',
            'sec-ch-ua-mobile': '?0',
            'sec-ch-ua-platform': '"Windows"',
            'sec-fetch-dest': 'empty',
   ...
   if  "search_text" in gen_link:
         response = requests.get('https://www.vinted.pl/api/v2/catalog/items', params=params, cookies={'_vinted_fr_session':x}, headers=headers).json()
   else:
         response = requests.get('https://www.vinted.pl/api/v2/catalog/items', params=params, cookies={'_vinted_fr_session':x} , headers=headers, proxies=proxies, auth=auth).json()  
         
   if catalog_id == [] and "search_text" not in gen_link:
         response = requests.get('https://www.vinted.pl/api/v2/catalog/items?catalog_ids=&color_ids=&brand_ids=&size_ids=&material_ids=&video_game_rating_ids=&status_ids=&is_for_swap=0&page={}&per_page=24&time=1662610671&search_session_id=62e41994-fbd4-4f54-ab9a-7dc591e9c59c'.format(page_id+1) , headers={'user-agent': f'{ua.random}'}, cookies={'_vinted_fr_session':x}, proxies=proxies, auth=auth).json() 
         ...
         for item in templates:
               if arr_stop[id] == "False":
                  ...
                  async with aiohttp.ClientSession(cookie_jar=aiohttp.CookieJar()) as s:
                     async with s.get("https://www.vinted.pl", headers={'user-agent': f'{ua.random}'}, proxy=str(proxy)) as r:
                        cookies = s.cookie_jar.filter_cookies('https://www.vinted.pl/')
                  ...
                  async with session.get(url=link_to_ad , headers=headers,  cookies=cookies) as response:
                     ...
                     try:
                        ...
                        file = open("otus_pl.txt", "w", encoding="utf-8")
                     ...
                     for i in number_of_views_to_ad:
                        try:
                           ...
                           pass
                     try:
                        chat_for_seller = "https://www.vinted.pl/"+soup.find(class_ = "c-button--default c-button--medium c-button c-button--truncated").get('href')
                        location_ad = soup.find_all(class_ = "details-list__item-value")
                        for loc in location_ad:
                           if "Polska" in loc.text:
                              location_ad = loc.text.strip()
                        if arr_stop[id] != "False":
                           if int(obv) >= int(number_of_ads_from_the_author) and int(pros) >= int(number_of_views_to_ad) and "Polska" in location_ad:
                              try:
