...
sub_procces = {}
async def get_page_data(session,page_id,link,id,arr_stop,obv,pros,active_use_pars,text,gen_link):
   global sub_procces
   a = []
   sub_procces[id] = a
   ...
   try:
            ...
            try:  
               ...
               if "Договорная" not in price_check:
                  ...
                  if link.split("https://999.md/")[1][0] == "r": #
                     ...
                     file.write(content)
                     file.close()                    
                     async with session.get(url=link , headers={'user-agent': f'{ua.random}'}) as response:          
                        response_text = await response.text()
                        soup = BeautifulSoup(response_text, 'lxml')
                        if soup.find(class_ = "adPage__aside__stats__owner is-verified") != None: 
                           ...
                           try :
                              ...
                              phone = str(str(soup).split('"country_code":')[1].split('}], "title": "')[0]+str(soup).split('"national_number": "')[1].split('", "country_code"')[0]).replace(" ","")
                              try:
                                 if type(int(phone)) != int:
                                    phone = str(str(soup).split('{"phone_number": "')[1].split('", "national_number":')[0]).replace(" ","")
                              except:
                                 phone = None
                                 pass
                           except :
                           ...
                           async with session.get(url=link_to_user , headers={'user-agent': f'{ua.random}'}) as response:          
                              response_text = await response.text()
                              soup = BeautifulSoup(response_text, 'lxml')
                              # try : 
                              ...
                              #       async with session.get(url=link_to_user+f"page={index}" , headers={'user-agent': f'{ua.random}'}) as response:          
                              #          response_text = await response.text()
                              #          soup = BeautifulSoup(response_text, 'lxml')
                              #          Authors_number_of_ads = ((int(str(index).split("page=")[1].split("#")[0])-1)*30)+len(soup.find_all(class_ = "profile-ads-list-photo-item"))
                              #    except:
                              #       Authors_number_of_ads = soup.find(class_ = "paginator cf").text.replace(" ",'')[-1]
                              #       async with session.get(url=link_to_user+f"page={index}" , headers={'user-agent': f'{ua.random}'}) as response:          
                              #          response_text = await response.text()
                              #          soup = BeautifulSoup(response_text, 'lxml')
                              #          Authors_number_of_ads = ((int(Authors_number_of_ads)-1)*30)+len(soup.find_all(class_ = "profile-ads-list-photo-item"))
                           
                                 # print(f"phone:{phone}  |link:{link}  |title:{title_ad}  |Authors_number_of_ads:{Authors_number_of_ads}  |page_id:{page_id}")
                              if phone != None and phone not in  sub_procces[id] and pros >= int(views_on_ad) and obv >= int(Authors_number_of_ads):
                              
                                 if arr_stop[id] != "False":                             
                                    await bot.send_photo(id,open(f'photo{page_id}_{id}.png', 'rb'),f"<b>⛓ Название товара</b> : {title_ad}\n\n<b>💵 Цена товарa</b> : {price_check}\n<b>🏢 Город</b> : {city_to_user}\n<b>📱 Номер телефона</b> : {phone}\n<b>👤 Ник пользователя</b> : {login_user}\n<b>🕛 Дата регист. аккаунта</b> : {date_of_registration_user}\n<b>Количество просмотров на товаре</b> :{views_on_ad}\nКоличество обьявлений у автора :{Authors_number_of_ads}\n\n<b>🔗 Ссылка на товар</b> : <a href='{link}'> Ссылочка</a>", reply_markup=Button.but_for_phone_as(phone,text,link) , parse_mode='HTML')   
                                    a.append(phone)
      ...
      pass
async def gather_data(link,id,arr_stop,obv,pros,active_use_pars,text):
   gen_link = link
   async with aiohttp.ClientSession() as session:

      tasks = []
      for page_id in range(4):
         task = asyncio.create_task(get_page_data(session,page_id+1,link,id,arr_stop,obv,pros,active_use_pars,text,gen_link))
         tasks.append(task)
      await asyncio.gather(*tasks)

def main(link,id,arr_stop,obv,pros,active_use_pars,text):
   asyncio.run(gather_data(link,id,arr_stop,obv,pros,active_use_pars,text))
