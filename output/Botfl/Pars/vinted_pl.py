...
def vinted_pl_pars(link,id,arr_stop,obv,pros,active_use_pars):
   current_page = 1
   all_page = 5 # all page
   gen_link = link
   ...
   def returns_arr_information(catalog_id,current_page,token,session):
      ua = UserAgent()
      response = requests.get(link, headers={'User-Agent': user_agent}) # Загружаем главную страницу для получения cookies
      ...
      headers = {
         ...
         'dnt': '1',
         'referer': 'https://www.vinted.pl/vetements?catalog[]='+str(catalog_id)+'&page='+str(current_page),
      ...
      if  "search_text" in gen_link:
         response = requests.get('https://www.vinted.pl/api/v2/catalog/items?search_text=iphone&catalog_ids=&color_ids=&brand_ids=&size_ids=&material_ids=&video_game_rating_ids=&status_ids=&is_for_swap=0&page='+str(current_page)+'&per_page=24&time=1662186434&search_session_id=0c23dbf6-982f-463b-9d30-404e2791d9a0'.format() ,headers={'User-Agent': user_agent}, cookies={'_vinted_fr_session':x}).json() 
      else:
